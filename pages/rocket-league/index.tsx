...
const ValorantHome = () => {
  ...
  useEffect(() => {
    if (!user) {
      ...
      axios
        .get(
          "https://guess-that-rank-server-production.up.railway.app/clips/get-all/rl"
        )
        .then((e) => {
          setIncompleteClips(
            e.data.filter((clip) => !guessedClips?.includes(clip.id))
          );
          setCompletedClips(
            e.data.filter((clip) => guessedClips?.includes(clip.id))
          );
    ...
    axios
      .get(
        `https://guess-that-rank-server-production.up.railway.app/guess/rl/not-guessed-clips/${user.nickname}`
      )
      .then((e) => {
    ...
    axios
      .get(
        `https://guess-that-rank-server-production.up.railway.app/guess/rl/guessed-clips/${user.nickname}`
      )
      .then((e) => {
  ...
  return (
    <div className={styles.root}>
      ...
      {tabSelected == "completed" && (
        <ClipGrid game="rocket-league" clips={completedClips} />
      )}
      {tabSelected == "incomplete" && (
        <ClipGrid game="rocket-league" clips={incompleteClips} />
      )}
    </div>
  );
};
